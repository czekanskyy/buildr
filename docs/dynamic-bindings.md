...
**Details.** `prepareRender(doc, registry, ctx, dataSource, { currentId?, concurrency?, cache? })` never throws. It inspects the top-level props of every node whose component is in the registry: a static `MediaRef` on a `media` prop and a static `{ type: 'query', spec }` on a `listSource` prop (a `binding` source needs no fetch). All media ids go out in one `getMedia(ids)`; queries run with at most 4 in flight, and identical resolved queries are issued once. Results land in `PreparedData.media` (by media id) and `PreparedData.queries` (by `queryKey(nodeId, prop)`); `collectionsUsed` is the sorted list of collections read. Failures become diagnostics tagged with `nodeId`/`prop` (`data.source-error`, `query.invalid`, `query.invalid-result`, `query.unresolved-value`, `query.depends-on-item`, `media.missing`, `media.invalid`) and the entry is left out. A filter operand that does not resolve fails the whole query rather than dropping the condition, because a dropped condition would widen the result. `excludeCurrent` needs `currentId`. Filters are limited to 4 nested groups and 32 nodes, and sorts to 4 keys.

The operator semantics every `DataSource` must share are pinned by the contract suite `runDataSourceContract` in `@next-buildr/test-utils` (strict typed comparisons; a missing field satisfies `neq`/`nin` but nothing else; missing values sort last; ties keep collection order). `createMemoryDataSource` is the reference implementation.
