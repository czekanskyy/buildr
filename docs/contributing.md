...
- Versioning uses a **fixed** Changesets group — every `@next-buildr/*` package is released together, at the same version, the same approach Payload itself uses (see [ADR-001](adr/ADR-001-monorepo.md)).
...
`apps/playground/e2e/visual` takes a screenshot of every gallery fixture at 375, 768 and 1280 px (`pnpm --filter @next-buildr/playground e2e:visual`) and compares it with the baselines in `e2e/visual/__screenshots__`. Fonts and anti-aliasing differ between operating systems, so the baselines are only ever taken in the Playwright Docker image, by the `Visual` workflow: a pull request without the `visual` label fails when a screenshot differs (the differences are uploaded as an artifact); a pull request **with** the `visual` label re-takes the baselines and commits them to the branch. Review the changed images in the diff before merging.

The same suite holds the **editor baseline** (PB-118): `e2e/visual/editor/editor.visual.spec.ts` screenshots the whole editor (empty page; a landing page with a Hero selected on the Content, Style and Advanced tabs; the layers tab expanded; the insert tab scrolled to the templates; the publish dialog; the media picker; the issues panel; the canvas at tablet and mobile width), each in light and dark and at 1280x800 and 1440x900, into `e2e/visual/__screenshots__/editor-*.png`. The states are seeded by the playground page `/?seed=empty|landing&theme=light|dark` (`apps/playground/src/visual-seeds.ts`; no `localStorage`, seeded node ids, and the editor's behaviour is unchanged), and every test waits for the canvas host's `data-status="ready"` (the `canvas:ready` handshake) before it takes the screenshot. An editor task that changes the look labels its pull request `visual` and reviews the changed `editor-*` images in the diff.

`e2e/a11y.spec.ts` runs axe (WCAG 2.0/2.1 A and AA) over the gallery in the normal `pnpm --filter @next-buildr/playground e2e`; the example application's pages are checked by `apps/example-next-payload/e2e/public.spec.ts`.
